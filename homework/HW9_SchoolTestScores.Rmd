...
The test scores have significantly improved because the scores from the 12 randomly selected classrooms are all above the previous mean of 72.6.
...
The mean of the new scores is higher than the previous averages. Another thing I noticed is that the standard deviation is much smaller than previous, meaning that the data is all clustered right around the mean. If I took a larger sample with more classroom scores, would they all still hover right around the mean or is this just a crazy coincidence? Another question is are we comparing the same classrooms from before because than the study wouldn't actually be proving the superintendent's points.
...
New program implementation increases reading scores
...
**Hypotheses:**
Null: The new reading program has not increased class scores
Alternative: The new reading program has increased class scores
...
Since the p-valus is below 0.05, we can reject the null hypothesis that the new reading program has not increased class scores. This test, however, doesn't prove that the new reading program is solely responsible for these changes. There could be other  factors that we don't know about playing a role on the increased scores.
...
My headline would be that the new reading program has increased classroom reading scores

Based off the data, there is evidence to support the superintendent's claim that the new reading program is increasing reading test scores. With a new mean score that is three higher than the previous number, the new reading program has had the desired effect on third grade students that it hoped to accomplish.
### Reflection Question 6:
What metrics or outcomes beyond test scores might be important to track for assessing reading performance?
- The difficulty of the standardized reading test (maybe the test is getting easier)
- The students' reading fluency of how smoothly and quickly they can get through a given block of text
