...
Recent property values in a local neighborhood have increased compared to previous house values, likely due to the new light rail line. After conducting a t test using the data collected, we are 95% confident that the new property values have spiked in relation to the prior house values from before the new light rail line was built. With a very miniscul p-value, we can safely reject the null hypothesis that new house values are actually lower than the prior property values. While there are many other factors that could lead to this, this change is likely due to the new light rail line.
